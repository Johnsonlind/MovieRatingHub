...
export default function AdminDetailViewsPage() {
  ...
  function toggleSelectAllCurrentPage() {
    ...
    const currentIds = data.items.map((it) => it.id);
  ...
  async function handleBatchDelete() {
    ...
    if (
      !window.confirm(
        `确定要删除选中的 ${selectedIds.length} 条访问记录吗？删除后无法恢复。`
      )
    ) {
      return;
    }
              ...
              <tr>
                <th className="text-left font-medium px-4 py-3 w-10">
                  <input
                    ...
                    checked={
                      !!data?.items?.length &&
                      data.items.every((it) => selectedIds.includes(it.id))
                    }

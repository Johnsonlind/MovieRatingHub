...
import { AdminMediaSearchResults } from '../../components/ui/AdminMediaSearchResults';
...
export default function AdminPlatformStatusPage() {
  ...
  useEffect(() => {
    async function loadMediaLocks() {
      ...
      try {
        ...
        const locked = new Set<string>(data.map((x) => x.platform.toLowerCase()));
  ...
  async function handleUnlock(item: MediaPlatformStatusItem) {
    ...
    setActionError('');
    try {
      const { record } = await unlockPlatformStatus({
  ...
  async function handleBulkLockUnlock(action: 'lock' | 'unlock') {
    ...
    try {
      ...
      setLockedPlatformsForMedia(new Set(latest.map((x) => x.platform.toLowerCase())));
      setSelectedPlatforms(new Set());
      void load();
    } catch (e: any) {

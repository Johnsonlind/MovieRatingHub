...
async function searchTMDB(q: string): Promise<SearchResult> {
  ...
  const res = await fetch(
    `/api/tmdb-proxy/search/multi?query=${encodeURIComponent(q)}&language=zh-CN`
  );
  const data = await res.json();
  const movies = (data.results || []).filter((r: any) => r.media_type === 'movie').map((r: any) => ({
    id: r.id,
    type: 'movie' as const,
    title: r.title || '',
...
export default function AdminRatingEditPage() {
  ...
  function getPlatformData(platformKey: string): Record<string, unknown> | null {
      ...
      <div className="space-y-6">
        <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50 p-4">
          ...
          <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto">
            {selectedMedia && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-200">
                {selectedMedia.poster && (
                  <img src={selectedMedia.poster} alt="" className="w-8 h-12 object-cover rounded" />
                )}
                <span>{selectedMedia.title} {selectedMedia.year && `(${selectedMedia.year})`}</span>
                <button
                  type="button"
            ...
            {!selectedMedia &&
              [...movies, ...tvs].slice(0, 12).map((item) => (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 hover:border-blue-400 text-left"
                >
                  {item.poster && (
                      ...
                      {displaySeasons.map((s, idx) => (
                        <div key={idx} className="p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 space-y-2">
                          ...
                          <div className="grid gap-2 sm:grid-cols-2">
                            {activePlatform === '豆瓣' && (
                              <>
                                <Input label="评分" value={String(s.rating ?? '')} onChange={(e) => { const src = seasons.length ? seasons : initialSeasons; setSeasons(src.map((x, i) => i === idx ? { ...x, rating: e.target.value } : x)); }} placeholder="8.5" />
                            ...
                            {activePlatform === 'TMDB' && (
                              <>
                                <Input label="评分" type="number" step="0.1" value={String(s.rating ?? '')} onChange={(e) => { const src = seasons.length ? seasons : initialSeasons; setSeasons(src.map((x, i) => i === idx ? { ...x, rating: e.target.value } : x)); }} placeholder="7.5" />
                                <Input label="投票数" type="number" value={String(s.vote_count ?? '')} onChange={(e) => { const src = seasons.length ? seasons : initialSeasons; setSeasons(src.map((x, i) => i === idx ? { ...x, vote_count: e.target.value } : x)); }} placeholder="500" />
                              </>
                            )}
                            {activePlatform === 'Trakt' && (
                              <>
                                <Input label="评分" type="number" step="0.1" value={String(s.rating ?? '')} onChange={(e) => { const src = seasons.length ? seasons : initialSeasons; setSeasons(src.map((x, i) => i === idx ? { ...x, rating: e.target.value } : x)); }} placeholder="8.2" />
                                <Input label="投票数" type="number" value={String(s.votes ?? '')} onChange={(e) => { const src = seasons.length ? seasons : initialSeasons; setSeasons(src.map((x, i) => i === idx ? { ...x, votes: e.target.value } : x)); }} placeholder="1000" />
                              </>
                            )}
                          </div>
                        </div>

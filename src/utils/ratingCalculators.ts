...
export function processDoubanRating(
  ...
  const getRating = season !== undefined
    ? (data: any) => {
        const seasonData = data.douban?.seasons?.find((s: any) => s.season_number === season);
        return seasonData?.rating || (season === 1 && data.douban?.rating && !data.douban?.seasons ? data.douban.rating : undefined);
      }
  ...
  const getVoteCount = season !== undefined
    ? (data: any) => {
        const seasonData = data.douban?.seasons?.find((s: any) => s.season_number === season);
        return seasonData?.rating_people || (season === 1 && data.douban?.rating_people && !data.douban?.seasons ? data.douban.rating_people : undefined);
      }

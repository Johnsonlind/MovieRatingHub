...
export function RatingSection({ 
  ...
  useEffect(() => {
    if (ratingData) {
      const newRating = calculateOverallRating(ratingData);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {realTimeRating?.rating && (
        <section className="mb-8">
          <h2 className="text-2xl font-bold mb-4">综合评分</h2>
          <OverallRatingCard 
            rating={realTimeRating.rating} 
            validPlatformsCount={realTimeRating.validRatings} 
          />
        </section>
      )}
      ...
      {/* 评分状态 */}
      <section className="mb-8">
        <h2 className="text-2xl font-bold mb-4 dark:text-white">数据来源</h2>

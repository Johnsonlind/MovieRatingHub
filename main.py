...
def build_manual_rating_cache_key(media_type: str, tmdb_id: int | str) -> str:
    """构造手工评分缓存的 Redis Key"""
    return f"rating:{media_type}:{tmdb_id}"


@app.get("/api/admin/manual-ratings/{media_type}/{tmdb_id}")
async def get_manual_rating(
    media_type: str,
    tmdb_id: int,
    current_user: User = Depends(get_current_user),
):
    ...
    cache_key = build_manual_rating_cache_key(media_type, tmdb_id)

    try:
        raw = await redis.get(cache_key)
        if not raw:
            raise HTTPException(status_code=404, detail="该影视无缓存数据")

        try:
            data = json.loads(raw)
        except Exception as e:
            logger.error(f"解析手工评分缓存失败: {e}")
        ...
        logger.error(f"获取手工评分缓存失败: {e}")
        raise HTTPException(status_code=500, detail=f"获取手工评分缓存失败: {str(e)}")


@app.post("/api/admin/manual-ratings/{media_type}/{tmdb_id}")
async def save_manual_rating(
    ...
    """
    保存手工评分数据并更新 Redis 缓存（仅管理员）
    
    前端应将表单数据整理为统一 JSON 结构，并作为请求体传入。
    服务端会自动补充 type / tmdb_id 字段，并写入:
      rating:{type}:{tmdbId}
    """
    require_admin(current_user)
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"请求体必须为合法的 JSON: {str(e)}")
    ...
    try:
        await redis.set(cache_key, json.dumps(body, ensure_ascii=False))
        return {
            "status": "success",
            "message": "手工评分已保存并更新缓存",
            "cache_key": cache_key,
        }
    except Exception as e:
        logger.error(f"保存手工评分缓存失败: {e}")
        raise HTTPException(status_code=500, detail=f"保存手工评分缓存失败: {str(e)}")
